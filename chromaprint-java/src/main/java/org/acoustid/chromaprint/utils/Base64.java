...
/**
 * {@literal Base64 encoder/decoder} implementation.
 *
 ...
 */
// TODO: Replace with Java implementation ({@link java.util.Base64})?
@Deprecated(since = "Use java.util.Base64", forRemoval = true)
public class Base64 {
	...
	public static String encode(byte[] data) {
		...
		StringBuilder stringBuilder = new StringBuilder(getBase64EncodedSize(data.length));
		int index = 0;

		while (index + 3 <= data.length) {
			int s0 = data[index++] & 0xFF;
			int s1 = data[index++] & 0xFF;
			int s2 = data[index++] & 0xFF;
			stringBuilder.append(BASE64_CHARS[(s0 >> 2) & 63]);
			stringBuilder.append(BASE64_CHARS[((s0 << 4) | (s1 >> 4)) & 63]);
			stringBuilder.append(BASE64_CHARS[((s1 << 2) | (s2 >> 6)) & 63]);
			stringBuilder.append(BASE64_CHARS[s2 & 63]);
		}
		...
		if (index + 2 == data.length) {
			...
			stringBuilder.append(BASE64_CHARS[(s0 >> 2) & 63]);
			stringBuilder.append(BASE64_CHARS[((s0 << 4) | (s1 >> 4)) & 63]);
			stringBuilder.append(BASE64_CHARS[(s1 << 2) & 63]);
		}
		else if (index + 1 == data.length) {
			int s0 = data[index++] & 0xFF;
			stringBuilder.append(BASE64_CHARS[(s0 >> 2) & 63]);
			stringBuilder.append(BASE64_CHARS[(s0 << 4) & 63]);
		}
